...
class IOR():
    '''Calculates IOR Rating for Defined Yacht. Default values are for Indulgence.'''
    ...
    def CGFCalc(s, ballastChange):
        ''' Calculates CGF based on a ballastChange = [weight, distance] '''

        CGF = (2.2 / ((34562.05963 / (871.13 - ballastChange[0] * ballastChange[1])) - 5.1)) + 0.8925
        s.actualCGF = CGF
        if (CGF >= 0.9680):
            return CGF
        else: 
    ...
    def Calc(s):
        ...
        s.MR = ((0.13 * s.L * s.SC)/n.sqrt(s.B * s.D) + 0.25 * s.L + 0.2 * s.SC + s.DC + s.FC) * s.DLF
    ...
    def ReqRMChange(s, reqRating):
        ...
        reqRMChange = 871.13 - (1 / (((1 / ((reqCGF - 0.8925) / 2.2)) + 5.1) / 34562.0593))
        return reqRMChange